...
export const getPricesProductSupermarket = async (req: Request, res: Response): Promise<void> => {
  try {
    const ids_products = typeof req.query.ids_products === 'string' ? req.query.ids_products : null;
    const id_supermarket = typeof req.query.id_supermarket === 'string' ? req.query.id_supermarket : null;
    const productsPricesSupermarket = await SupermercadoProductoModel.getPricesProductSupermarket(id_supermarket, ids_products);
    res.status(200).json(productsPricesSupermarket);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Error interno del servidor' });
  }
